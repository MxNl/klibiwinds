...
make_plot_maps_hex <- function(plot_data, regions, absolute_value_or_change = "change") {
  ...
  plot_data_grid <- plot_data_grid |>
    sf::st_join(plot_data) |>
    dplyr::filter(!is.na(well_id)) |>
    sf::st_drop_geometry() |>
    dplyr::group_by(hex_id, name, period) |>
    dplyr::summarise(value = mean(value)) |>
    dplyr::inner_join(x = plot_data_grid)
